# Combined Dockerfile for hermes-agent
# This file combines Dockerfile.base and Dockerfile.app into a single build stage.
#
# Build command:
...
# empty placeholder — the real README is restored by the final COPY step.
#
...
#
# The editable link is created after the full source copy.
COPY pyproject.toml uv.lock ./
RUN touch ./README.md
RUN uv sync --frozen --no-install-project --extra all

# ---------- Permissions (all owned by hermes) ----------
# Make install dir world-readable so any HERMES_UID can read it at runtime.
# The venv needs to be traversable too.
# All directories need to be writable by the hermes user
...
# ---------- Runtime (same for all images FROM this base) ----------
ENV HERMES_WEB_DIST=/opt/hermes/hermes_cli/web_dist
ENV HERMES_HOME=/opt/data
ENV PATH="/opt/data/.local/bin:${PATH}"
VOLUME [ "/opt/data" ]

# ========== Application layer (from Dockerfile.app) ==========
# Copy source code and perform app-specific build steps
